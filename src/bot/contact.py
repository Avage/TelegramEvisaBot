...
def send_help_menu(bot, db, message):
    if not db.user_exist(message.chat):
        faq_button = types.InlineKeyboardButton(msg('en', 'faq_button'), callback_data='faq')
        contact_with_admin = types.InlineKeyboardButton(msg('en', 'contact_admin_button'),
                                                        callback_data='admin')
        back_button = types.InlineKeyboardButton(msg('en', 'back_button'), callback_data='menu')
        keyboard = types.InlineKeyboardMarkup()
        keyboard.add(faq_button, contact_with_admin, back_button, row_width=2)
        bot.send_message(message.chat.id, msg('en', 'help_message'), reply_markup=keyboard,
                         parse_mode='Markdown')
        ...
        faq_button = types.InlineKeyboardButton(msg(db.get_lg(message.chat), 'faq_button'),
                                                callback_data='faq')
